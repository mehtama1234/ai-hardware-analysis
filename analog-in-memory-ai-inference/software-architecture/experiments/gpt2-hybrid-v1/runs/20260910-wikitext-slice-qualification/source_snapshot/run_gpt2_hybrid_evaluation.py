...
def evaluate(model, tokenizer, texts, new_tokens, projection=None):
    results = []
    for text in texts:
        ...
        results.append({"logits": logits, "token_count": target.numel(), "nll_sum": float(nll),
                        ...
                        "cpu_reference_wall_ms": (time.perf_counter() - started) * 1000})
    return results
...
def compare(baseline, candidate):
    ...
    for ref, test in zip(baseline, candidate):
        ...
        rows.append({"predicted_tokens": ref["token_count"], "argmax_matches": equal,
                     ...
                     "baseline_cpu_reference_wall_ms": ref["cpu_reference_wall_ms"],
                     "candidate_cpu_reference_wall_ms": test["cpu_reference_wall_ms"]})
...
def run(args):
    ...
    snapshot = Path(snapshot_download(fixture["model_id"], revision=fixture["revision"], local_files_only=True))
    ...
    for name, profile, ideal in [
        ...
        with replace_forward(module, lambda x: projection(x, ideal=ideal)):
            candidate = evaluate(model, tokenizer, fixture["evaluation"], fixture["max_new_tokens"], projection)
    ...
    fallback = compare(baseline, evaluate(model, tokenizer, fixture["evaluation"], fixture["max_new_tokens"]))
    ...
    report = {
        ...
        "runtime": {"python": sys.version, "torch": torch.__version__, "transformers": transformers.__version__,
                    "platform": platform.platform(), "device": "cpu", "threads": args.threads,
                    "command": sys.argv},
        "fixture": fixture,
        "variants": variants,
