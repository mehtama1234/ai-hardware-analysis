...
def run(args):
    ...
    snapshots.mkdir()
    for name in [Path(__file__).name, "run_gpt2_hybrid_evaluation.py", "tiled_projection_model.py", "projection_numerical_control.py"]:
    ...
    with torch.inference_mode(), (args.output / "rows.jsonl").open("w") as stream:
        for index, (start, ids) in enumerate(selections["test"]):
            ...
            native_outputs = []
            hook = module.register_forward_hook(lambda _m, _inputs, value: native_outputs.append(value.clone()))
            try:
            ...
            for name, _, ideal in variants:
                ...
                candidate_outputs = []
                def forward(x):
                    value = projection(x, ideal=ideal)
                    if ideal:
                        candidate_outputs.append(value.clone())
                    return value
                ...
                if ideal:
                    row["numerical_control"] = assess(native_outputs[0], candidate_outputs[0], baseline, candidate, target)
                rows.append(row)
