#!/usr/bin/env python3
"""Validation-only comparison using frozen nominal circuit reference settings."""
import argparse
...
def main():
    ...
    with torch.inference_mode(), (args.output / 'rows.jsonl').open('w') as stream:
        for index, window in enumerate(protocol['windows']):
            ...
            print(f'Completed validation context {index+1}/{len(protocol["windows"])}', flush=True)
    ...
    result = dict(schema_version='circuit_reference_validation.v1', variants=summaries, fallback_controls=fallback,
                  ideal_control_pass=True, evaluation_split='validation', analog_authorized=False,
