...
from check_gpt2_reference_validation import check as check_reference
...
def run(args):
    ...
    reference_holdout = quality.get("schema_version") == "circuit_reference_disjoint_holdout.v1"
    heldout = reference_holdout or quality.get("schema_version") == "calibrated_adc_disjoint_holdout.v1"
    if quality.get("schema_version") == "circuit_reference_validation.v1":
        raise ValueError("Validation development evidence cannot replace the test-set decision package")
    if reference_holdout:
        check_reference(args.quality)
        plan = json.loads((args.quality / "frozen_test_plan/plan.json").read_text())
    ...
    if heldout:
        inputs["quality_plan"] = args.quality / ("frozen_test_plan/plan.json" if reference_holdout else "frozen_plan/plan.json")
    if reference_holdout:
        inputs["nominal_reference_binding"] = args.quality / "binding.json"
    ...
    for name in [Path(__file__).name, "check_gpt2_wikitext_projection.py", "check_gpt2_adc_holdout.py",
                 "check_gpt2_reference_validation.py", "freeze_reference_holdout.py"]:
    ...
    report = {"schema_version": "gpt2_hybrid_evidence_decision.v1", "decision": "retain_native_digital_execution",
              ...
              "array_model_circuit_calibrated": quality["physical_profile_qualified"] if reference_holdout else quality["physical_profile_calibrated"],
    ...
    if reference_holdout:
        report["quality"]["scope"] = "Frozen finite-reference profile on 32 test contexts disjoint from 64 earlier contexts; static noiseless model"
        report["reference_component"] = {
            "scope": "Finite ADC ranges derived from nominal SKY130-switch DC reference curve",
            "physical_array_or_converter_qualification": False,
            "dynamic_reference_settling_in_model": False,
            "note": "A static reference-component curve does not establish a circuit-calibrated array error model"}
    if heldout:
