...
def run(args):
    ...
    for item in manifest["files"]:
        checked_source(item)
    if compiler["model"]["revision"] != quality["protocol"]["model_revision"]:
    ...
    for name in [Path(__file__).name, "check_gpt2_wikitext_projection.py"]:
    ...
    report = {"schema_version": "gpt2_hybrid_evidence_decision.v1", "decision": "retain_native_digital_execution",
              "model_revision": quality["protocol"]["model_revision"], "projection": compiler["model"]["target_module"],
