...
class CompiledProjection:
    ...
    def __init__(self, package, weight, bias, physical_gate, expected_hashes):
        ...
        repair = physical_gate.get("physical_repair_evidence")
        connected = repair.get("final_preamp_connections_pass", False) if repair else physical_gate.get("active_macro_candidate", {}).get("physical_preamp_routing_complete", False)
        if not connected:
            self.fallback_reasons.append("final_netlist_preamp_path_unverified")
        if repair and not repair.get("full_cell_drc_pass", False):
            self.fallback_reasons.append("full_cell_layout_drc_failed")
