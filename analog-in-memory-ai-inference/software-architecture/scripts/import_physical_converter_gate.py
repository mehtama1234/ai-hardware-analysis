...
def import_gate(path: Path, starter_area_path: Path | None = None) -> dict[str, Any]:
    ...
    repair_path = DEFAULT_GATE.parent / "converter-physical-repair-current.json"
    repair = None
    if repair_path.exists():
        repair = json.loads(repair_path.read_text())
        for evidence_source in repair["sources"].values():
            if sha256(Path(evidence_source["path"])) != evidence_source["sha256"]:
                raise ValueError("physical repair evidence source hash mismatch")
        repair = {**repair, "source":str(repair_path), "source_sha256":sha256(repair_path)}
    return {
        ...
        "physical_repair_evidence": repair,
        "analog_allowed_for_physical_claim": bool(source.get("ready_for_candidate_post_layout_payload", False)) and bool(repair and repair.get("analog_allowed_for_physical_claim") is True),
        "next_actions": [
