...
def compare(actual, expected, *, atol, rtol):
    ...
    return {'passed': finite and bool((error <= bound).all()), 'finite': finite,
        'elements': actual.numel(), 'max_abs_error': float(error.max()),
        'outside_tolerance': int((error > bound).sum()), 'atol': atol, 'rtol': rtol}
