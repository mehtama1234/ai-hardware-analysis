...
def main():
    ...
    p.add_argument('--candidate', choices=('recomputed', 'native_chunked'), default='recomputed')
    p.add_argument('--reference-chunk-size', type=int, default=0,
        help='zero uses full native projection; positive values isolate custom autograd against matching native chunks')
    p.add_argument('--scope', choices=('all', 'head_and_last_block'), default='all')
    args = p.parse_args()
    if not 1 <= args.sequence <= 1024 or args.chunk_size < 1 or args.reference_chunk_size < 0:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='gpt2-training-parity-') as tmp:
            ...
            for mode in ('standard', 'recomputed'):
                ...
                if args.scope == 'head_and_last_block':
                ...
                if mode == 'standard':
                    if args.reference_chunk_size:
                        chunk = args.reference_chunk_size
                        loss = sum(F.cross_entropy(model.lm_head(hidden[i:i + chunk]),
                            labels[i:i + chunk], reduction='sum')
                            for i in range(0, len(labels), chunk)) / len(labels)
                    else:
                        loss = F.cross_entropy(model.lm_head(hidden), labels)
                elif args.candidate == 'native_chunked':
                    loss = sum(F.cross_entropy(model.lm_head(hidden[i:i + args.chunk_size]),
                        labels[i:i + args.chunk_size], reduction='sum')
                        for i in range(0, len(labels), args.chunk_size)) / len(labels)
                else:
                ...
                for i, (name, parameter) in enumerate((item for item in model.named_parameters() if item[1].requires_grad)):
