...
def main() -> int:
    ...
    artifact = {
        "project": "hf-quant-serving",
        "track": "Hugging Face serving baseline",
        "profile": "quantized-numerics",
        ...
        "measurement": {
            ...
            "summary": f"Starter {status} for Hugging Face serving baseline; return code {proc.returncode}.",
        },
        "source": "hf_baseline.py",
        "environment": "local",
        "baseline_measurement": {"path": "21-gpumode-quantized-kernels/out_gpumode_quantized_kernels.json", "summary": "Weight-only quantization reduces modeled memory traffic, but this CPU proxy also exposes the dequantization tax that a real GPU kernel must fuse away to turn smaller weights into speed."},
        "starter": {
