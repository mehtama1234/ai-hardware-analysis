...
def main() -> int:
    ...
    profiler = json.loads((args.reports_dir / "profiler-evidence.json").read_text())
    ...
    require(all(checks.get(f"uncached_preallocated_output_parity_{level}") is True for level in [1, 2, 4]), "preallocated serving output parity failed")
