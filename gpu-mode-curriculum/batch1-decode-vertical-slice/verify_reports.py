...
def main() -> int:
    ...
    if serving.get("device") == "cuda":
        ...
        require(serving.get("cuda_graph_microbatch") is not None, "CUDA Graph microbatch report missing")
        require(checks.get("cuda_graph_microbatch_vectorized_observed") is True, "CUDA Graph vectorized batch missing")
        require(all(row.get("accepted") == 8 for row in serving["cuda_graph_microbatch"]["rows"]), "CUDA Graph microbatch requests incomplete")
        require(all(checks.get(f"cuda_graph_microbatch_output_parity_{key}") is True for key in keys), "CUDA Graph microbatch output parity failed")
