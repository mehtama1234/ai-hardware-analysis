...
def category(name):
    lower = name.lower()
    if any(x in lower for x in ('attention', 'softmax', 'paged', 'bmm')):
        return 'attention'
...
def capture(model, encoded, path):
    ...
    for event in profiler.key_averages():
        ...
        # Aggregate ATen operators only: runtime/kernel rows and the enclosing
        # user annotation overlap their device attribution.
