...
def decode_start_offsets(attention_mask, batch, device):
    """Locate the first allowed KV entry without a CPU-visible nonzero shape."""
    if attention_mask is None:
        return torch.zeros((batch,), dtype=torch.int32, device=device)
    rows = attention_mask.reshape(batch, -1, attention_mask.shape[-1])[:, -1, :]
    allowed = rows if rows.dtype == torch.bool else rows > -1.0e4
    # argmax returns zero for an entirely masked row, matching the previous
    # empty-nonzero behavior. Nonempty left-padded prompts are the supported path.
    return allowed.to(dtype=torch.int32).argmax(dim=-1).to(dtype=torch.int32)
...
def install_fused_attention(model, module, persistent=False):
    ...
    def fused(attention, query, key, value, attention_mask, scaling=None, dropout=0.0, **kwargs):
        ...
        starts = decode_start_offsets(attention_mask, batch, key.device)
