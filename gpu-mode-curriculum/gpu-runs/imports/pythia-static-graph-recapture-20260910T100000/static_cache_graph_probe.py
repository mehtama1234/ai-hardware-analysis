...
def main():
    ...
    parser.add_argument('--reuse-policy', choices=('reuse', 'recapture'), default='reuse')
    ...
    with torch.inference_mode():
        ...
        cache = StaticCache(config=model.config, max_cache_len=max_len)
        prefill_ids = encoded['input_ids']
        position_ids = (mask[:, :width].long().cumsum(-1) - 1)
        position_ids.masked_fill_(mask[:, :width] == 0, 1)
        cache_position = torch.arange(width, device=device)
        out = model(input_ids=prefill_ids, attention_mask=mask[:, :width], position_ids=position_ids,
                    cache_position=cache_position, past_key_values=cache, use_cache=True)
        ...
        torch.cuda.synchronize()
        # Reuse the same captured cache and tensor addresses for a second
        # fixed-shape bucket. StaticCache.reset() clears logical length while
        # preserving storage, which is the lifecycle required for slot reuse.
        second_ids = encoded['input_ids'].clone()
        second_ids[:, -1] = (second_ids[:, -1] + 1) % model.config.vocab_size
        eager_second = run_eager(model, second_ids, encoded['attention_mask'], width, args.steps, max_len)
        cache.reset()
        position_ids = (mask[:, :width].long().cumsum(-1) - 1)
        position_ids.masked_fill_(mask[:, :width] == 0, 1)
        cache_position = torch.arange(width, device=device)
        out_second = model(input_ids=second_ids, attention_mask=mask[:, :width], position_ids=position_ids,
                           cache_position=cache_position, past_key_values=cache, use_cache=True)
        ids.copy_(out_second.logits[:, -1].argmax(-1, keepdim=True))
        position_ids.fill_(width)
        cache_position.fill_(width)
        reused_tokens = [ids[:, 0].clone()]
        reuse_graph = graph
        reuse_output = graph_out
        recapture_ms = None
        if args.reuse_policy == 'recapture':
            recapture_start = time.perf_counter()
            reuse_graph = torch.cuda.CUDAGraph()
            with torch.cuda.graph(reuse_graph):
                reuse_output = model(input_ids=ids, attention_mask=mask, position_ids=position_ids,
                                     cache_position=cache_position, past_key_values=cache, use_cache=True)
            recapture_ms = (time.perf_counter() - recapture_start) * 1000
        for step in range(args.steps - 1):
            reuse_graph.replay()
            next_ids = reuse_output.logits[:, -1].argmax(-1).clone()
            reused_tokens.append(next_ids)
            ids.copy_(next_ids.unsqueeze(1))
            position_ids.fill_(width + step + 1)
            cache_position.fill_(width + step + 1)
        reused_tokens = torch.stack(reused_tokens, dim=1)
        torch.cuda.synchronize()
    report = {'schema_version': 'static-cache-cuda-graph-probe-v0.1', 'status': 'passed',
        ...
        'checks': {'token_parity': bool(torch.equal(eager, graph_tokens)),
                   'slot_reuse_parity': bool(torch.equal(eager_second, reused_tokens)),
        ...
        'metrics': {'peak_allocated_bytes_after_capture': torch.cuda.max_memory_allocated(),
                    ...
                    'reuse_policy': args.reuse_policy, 'recapture_ms': recapture_ms},
        'tokens': {'eager': eager.tolist(), 'graph': graph_tokens.tolist(),
                   'eager_reuse': eager_second.tolist(), 'graph_reuse': reused_tokens.tolist()},
