...
class ContinuousScheduler(StreamingScheduler):
    def __init__(self, backend, *, slots=4, max_pending=32, max_tokens=16):
        self.live = {}
        self.admissions = []
        self.worker_timing = {'admit_calls': 0, 'admit_wall_ms': 0.0,
                              'tick_calls': 0, 'tick_wall_ms': 0.0,
                              'inter_tick_gap_ms': 0.0, 'max_inter_tick_gap_ms': 0.0}
    ...
    def _admit(self, request):
        ...
        try:
            ...
            self.worker_timing['admit_calls'] += 1
            self.worker_timing['admit_wall_ms'] += (time.perf_counter() - start) * 1000
        ...
        while True:
            ...
            try:
                ...
                if self._last_tick_end is not None:
                    gap_ms = (start - self._last_tick_end) * 1000
                    self.worker_timing['inter_tick_gap_ms'] += gap_ms
                    self.worker_timing['max_inter_tick_gap_ms'] = max(
                        self.worker_timing['max_inter_tick_gap_ms'], gap_ms)
                events = self.backend.tick()
                elapsed = (time.perf_counter() - start) * 1000
                self.worker_timing['tick_calls'] += 1
                self.worker_timing['tick_wall_ms'] += elapsed
    ...
    def snapshot(self):
        ...
        with self.lock:
            ...
            state['worker_timing'] = dict(self.worker_timing)
        return state
