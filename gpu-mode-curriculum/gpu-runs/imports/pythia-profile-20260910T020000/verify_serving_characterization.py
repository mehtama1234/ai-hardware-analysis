...
def main():
    ...
    if d.get('status') != 'passed': errors.append('report status')
    ...
    for row in rows:
        if row.get('output_parity') is not True or row.get('cached_reference_parity') is not True:
            errors.append(f"cached/reference parity batch {row.get('batch_size')}")
        if row.get('reference_token_ids') != row.get('cached_token_ids'):
            errors.append(f"cached token trace batch {row.get('batch_size')}")
