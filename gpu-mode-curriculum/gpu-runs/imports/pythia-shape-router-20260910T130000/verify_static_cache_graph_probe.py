...
def validate(report, model_id, revision):
    ...
    check(checks.get('token_parity') is True and checks.get('shape_stable') is True and
          checks.get('graph_replayed') is True, 'capture checks')
    if 'slot_reuse_parity' in checks:
        check(checks.get('slot_reuse_parity') is True, 'slot reuse checks')
