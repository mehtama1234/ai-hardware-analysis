...
def load(engine, tokenizer, references, rate, seconds, run, scheduler_factory=None):
    if hasattr(engine, 'reset_profile'):
    ...
    if hasattr(scheduler.backend, 'reset_profile'):
        scheduler.backend.reset_profile()
    server = make_server(scheduler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    ...
    result = {'offered_rate': rate, 'duration_requested_s': seconds, 'round': run,
    ...
    if hasattr(engine, 'profile_snapshot'):
        result['profile'] = engine.profile_snapshot()
    if 'worker_timing' in state:
        result['scheduler_profile'] = state['worker_timing']
    if hasattr(scheduler.backend, 'profile_snapshot'):
        result['backend_profile'] = scheduler.backend.profile_snapshot()
    return result
...
def main():
    ...
    parser.add_argument('--profile', action='store_true', help='collect synchronized HF slot timing counters')
    ...
    report = {'schema_version': 'continuous-arrival-load-v0.1', 'evidence_kind': 'measured_gpu',
        ...
        'runs': rows, 'eos_probe': eos, 'profiling': bool(args.profile), 'limitations': ['Bounded loopback arrival windows, not production capacity.',
