...
def main():
    ...
    with torch.inference_mode():
        refs = {(x, budget): model.generate(**tok([x], return_tensors='pt').to('cuda'),
    ...
    try:
        with ThreadPoolExecutor(max_workers=8) as pool:
            futures = [pool.submit(post, f'http://127.0.0.1:{server.server_port}',
                f'bucket-{round_index}-{budget}-{i}', prompt, budget)
    ...
    for round_index, rows in enumerate(rounds):
        for (source_round, source_i, prompt, budget), row in zip(primary_plan, rows):
            ...
            outputs.append({'round': source_round, 'request': source_i, 'budget': budget,
    ...
    checks = {'four_shape_budget_classes': len(scheduler.schedulers) == 4,
              ...
              'token_parity': all(x['tokens'] == refs[(x['prompt'], x['budget'])] for x in outputs),
              ...
              'four_recaptures': sum(len(s.backend.bucket_meta) for s in scheduler.schedulers.values()) >= 4}
    ...
    report = {'schema_version': 'static-graph-http-v0.1', 'evidence_kind': 'measured_gpu',
        ...
        'protocol': {'shape_classes': 2, 'output_budgets': [4, 8], 'primary_requests': 8,
