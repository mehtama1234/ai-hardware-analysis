...
def load(engine, tokenizer, references, rate, seconds, run):
    scheduler = ContinuousScheduler(SlotBackend(engine, tokenizer), slots=2, max_pending=8, max_tokens=32)
    server = make_server(scheduler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
...
def main():
    ...
    hashes = {}
    for name in ('graph_decode.py','slot_decode.py','continuous_service.py','continuous_http.py','real_model_service.py','run_continuous_load.py'):
    ...
    report = {'schema_version': 'continuous-arrival-load-v0.1', 'evidence_kind': 'measured_gpu',
        ...
        'source_hashes': hashes, 'prompts': PROMPTS, 'references': references,
