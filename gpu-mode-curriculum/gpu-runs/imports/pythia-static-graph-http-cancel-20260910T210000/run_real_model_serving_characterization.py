    ...
    def decode():
        ...
        with torch.inference_mode():
            for _ in range(MAX_NEW_TOKENS - 1):
                ...
                past_length = past.get_seq_length() if hasattr(past, 'get_seq_length') else past[0][0].shape[-2]
                ...
                out = model(input_ids=tokens[-1].unsqueeze(-1), attention_mask=decode_mask,
                            position_ids=decode_positions, past_key_values=past,
...
def measure_batch(model, tokenizer, prompts: list[str], batch_size: int) -> dict:
    # Keep every row's prompt width identical. This isolates cache/batch scaling
    # from architecture-specific left-padding semantics (notably GPT-NeoX).
    selected = [prompts[0]] * batch_size
    ...
    for _ in range(REPEATS):
        ...
        parity = parity and torch.equal(cached_tokens, reference_tokens)
    return {
        ...
        "cached_reference_parity": parity,
        "uncached_reference_parity": torch.equal(uncached_tokens, reference_tokens),
        "reference_token_ids": reference_tokens.detach().cpu().tolist(),
        "cached_token_ids": cached_tokens.detach().cpu().tolist(),
        "uncached_token_ids": uncached_tokens.detach().cpu().tolist(),
        "cached": {
        ...
        "scope": "manual cached/uncached prefill plus autoregressive decode on homogeneous prompt batches; cached path is checked against Transformers generate; CUDA synchronized; allocator peak includes model and activations; phase fields are last measured repeat",
    }
...
def main() -> int:
    ...
    report = {
        ...
        "status": "passed" if all(row["output_parity"] for row in rows) else "failed",
        "model_profile": {
        ...
        "protocol": {"workloads": list(PROMPTS), "batch_sizes": [1, 2, 4], "max_new_tokens": MAX_NEW_TOKENS, "repeats": REPEATS, "generation": "greedy", "padding_policy": "homogeneous prompt replication per batch", "timing": "torch.cuda.synchronize before and after each sample"},
        "rows": rows,
        "claim_boundary": {
    ...
    parity = all(row["output_parity"] for row in rows)
    print(json.dumps({"status": report["status"], "output": str(output), "device": report["device_name"], "rows": len(rows), "parity": parity}, indent=2))
