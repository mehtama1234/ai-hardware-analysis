...
class BucketBackend:
    def __init__(self, model, tokenizer, bucket, cancel_probe_delay_ms=0):
        self.model, self.tokenizer, self.bucket = model, tokenizer, bucket
        self.cancel_probe_delay_ms = cancel_probe_delay_ms
        self.bucket_meta = []
    ...
    def batch_events(self, requests):
        ...
        for index in range(tokens.shape[1]):
            if self.cancel_probe_delay_ms:
                time.sleep(self.cancel_probe_delay_ms / 1000)
...
class ShapeBucketRouter:
    ...
    def __init__(self, model, tokenizer, cancel_probe_delay_ms=0):
    ...
    def submit(self, request_id, prompt, max_new_tokens=None):
        ...
        if scheduler is None:
            ...
            backend = BucketBackend(self.model, self.tokenizer, bucket, self.cancel_probe_delay_ms)
...
def stream_and_cancel(url, request_id, prompt, max_new_tokens):
    """Read one token from a stream, cancel it over a second HTTP request."""
    body = json.dumps({'request_id': request_id, 'prompt': prompt,
                       'max_new_tokens': max_new_tokens}).encode()
    try:
        request = urllib.request.Request(url + '/stream', body,
            headers={'Content-Type': 'application/json'})
        with urllib.request.urlopen(request, timeout=60) as response:
            first = json.loads(next(response))
            cancel_body = json.dumps({'request_id': request_id}).encode()
            cancel_request = urllib.request.Request(url + '/cancel', cancel_body,
                headers={'Content-Type': 'application/json'})
            with urllib.request.urlopen(cancel_request, timeout=10) as cancel_response:
                cancel_result = json.load(cancel_response)
            events = [first] + [json.loads(line) for line in response]
        return {'status': response.status, 'events': events, 'cancel': cancel_result}
    except Exception as exc:
        return {'status': None, 'error': f'{type(exc).__name__}: {exc}'}
...
def main():
    ...
    p.add_argument('--cancel-probe', action='store_true')
    ...
    scheduler = ShapeBucketRouter(model, tok, cancel_probe_delay_ms=20 if a.cancel_probe else 0)
    ...
    try:
        ...
        cancellation = stream_and_cancel(f'http://127.0.0.1:{server.server_port}',
                                         'cancel-probe', prompts[0], 8) if a.cancel_probe else None
    finally:
    ...
    cancellation_tokens = ([e['token_id'] for e in cancellation.get('events', [])
                            if e.get('type') == 'token'] if cancellation else [])
    ...
    if a.cancel_probe:
        checks.update({'cancellation_http_ok': cancellation.get('status') == 200,
                       'cancellation_accepted': cancellation.get('cancel', {}).get('cancelled') is True,
                       'cancellation_terminal': any(e.get('type') == 'done' and
                                                    e.get('status') == 'cancelled_inflight'
                                                    for e in cancellation.get('events', [])),
                       'cancellation_partial': 0 < len(cancellation_tokens) < 8})
    ...
    report = {'schema_version': 'static-graph-http-v0.1', 'evidence_kind': 'measured_gpu',
        ...
        'cancellation': ({'status': cancellation.get('status'), 'cancel': cancellation.get('cancel'),
                          'token_count': len(cancellation_tokens),
                          'terminal': next((e for e in cancellation.get('events', [])
                                            if e.get('type') == 'done'), None)} if cancellation else None),
