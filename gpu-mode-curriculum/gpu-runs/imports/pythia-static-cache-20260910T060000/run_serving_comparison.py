...
def main():
    ...
    parser.add_argument('--profile', action='store_true', help='collect per-mode execution timing counters')
    parser.add_argument('--native-cache-kind', choices=('dynamic', 'static'), default='dynamic')
    args = parser.parse_args()
    if not torch.cuda.is_available() or args.seconds < 4:
        raise SystemExit('CUDA and >=4-second windows required')
    args.output_dir.mkdir(parents=True, exist_ok=True)
    torch.backends.cuda.matmul.allow_tf32 = False
    tokenizer = AutoTokenizer.from_pretrained(args.model_id, revision=args.revision, padding_side='left')
    ...
    engine = HFSlotDecode(model, 2, 256, profile=args.profile) if args.engine == 'hf' else SlotDecode(model, 2, 256)
    engine.capture()
    factories = {
        'native_microbatch': lambda: MicrobatchScheduler(NativeBatchBackend(model, tokenizer, profile=args.profile, cache_kind=args.native_cache_kind)),
    ...
    for name in ('graph_decode.py','slot_decode.py','continuous_service.py','continuous_http.py',
                 'real_model_service.py','run_continuous_load.py','microbatch_control.py','run_serving_comparison.py','hf_slot_decode.py'):
    ...
    manifest.write_text(json.dumps({'source_hashes':hashes,'requirements_sha256':hashlib.sha256(requirements.read_bytes()).hexdigest()},indent=2)+'\n')
    bundle=args.output_dir/'serving-source.tar.gz'
    with tarfile.open(bundle,'w:gz') as archive:
        for path in [*(sources/name for name in hashes),requirements,manifest]:
            archive.add(path,arcname=path.name)
    report = {'schema_version':'matched-serving-v0.2','evidence_kind':'measured_gpu',
        'status':'passed' if all(all(r['checks'].values()) for r in runs) else 'failed',
        'model_id':args.model_id,'model_revision':args.revision,'tokenizer_revision':args.revision,'source_hashes':hashes,
        'engine': args.engine, 'profiling': bool(args.profile),
        ...
        'prompts':PROMPTS,'references':references,'protocol':{'modes':modes,'rates':[16,48],
            ...
            'replay_criterion': ('At rate 48, continuous median goodput exceeds both controls; at rate 16, continuous median window completion p95 is below graph groups. Reproduce correctness and these directions, not absolute latency equality.' if args.engine == 'gpt2' else 'Reproduce correctness and per-mode metrics for this architecture; no performance advantage is presupposed.'),
            ...
            'execution_note': ('HF generic modes use eager per-slot model calls; they are graph scheduler controls, not CUDA Graph captures.' if args.engine == 'hf' else 'GPT-2 graph modes use the captured fixed-shape CUDA graph where enabled.'),
            'native_cache_kind': args.native_cache_kind},
