    ...
    def decode():
        ...
        with torch.inference_mode():
            for _ in range(MAX_NEW_TOKENS - 1):
                ...
                out = model(input_ids=tokens[-1].unsqueeze(-1), attention_mask=decode_mask, past_key_values=past, use_cache=True)
                past = out.past_key_values
...
def generate_uncached(model, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> tuple[torch.Tensor, float, float]:
    ...
    def decode():
        ...
        with torch.inference_mode():
            for _ in range(MAX_NEW_TOKENS - 1):
                out = model(input_ids=sequence, attention_mask=decode_mask, use_cache=False)
...
def main() -> int:
    ...
    print(json.dumps({"status": "passed", "output": str(REPORT), "device": report["device_name"], "rows": len(rows), "parity": all(row["output_parity"] for row in rows)}, indent=2))
    return 0
