...
if [[ "${MODE}" == "paged-kv" || "${MODE}" == "paged-attention" || "${MODE}" == "serving-tail" || "${MODE}" == "digits-quality" || "${MODE}" == "eager-kernels" || "${MODE}" == "wmma-profiler" ]]; then
  DEST="${ROOT_DIR}/gpu-runs/imports/${RUN_ID}"
  mkdir -p "${DEST}"
  if [[ "${MODE}" == "paged-attention" ]]; then
