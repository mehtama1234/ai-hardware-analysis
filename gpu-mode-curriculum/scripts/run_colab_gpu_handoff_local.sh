...
echo "Downloading Colab artifacts ..."
  ...
  if [[ "${MODE}" == "trained-e2e" ]]; then
    ...
    colab --auth oauth2 download -s "${SESSION_NAME}" "/content/gpu-mode-curriculum/model-integration/reports/serving-tail-load-cuda.json" "${DEST}/serving-tail-load-cuda.json"
  elif [[ "${MODE}" == "batch1-graphs" ]]; then
