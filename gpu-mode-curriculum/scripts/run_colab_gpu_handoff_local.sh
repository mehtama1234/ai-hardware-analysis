...
if [[ "${MODE}" == "paged-kv" || "${MODE}" == "paged-attention" || "${MODE}" == "serving-tail" || "${MODE}" == "serving-tail-graphs" || "${MODE}" == "digits-quality" || "${MODE}" == "eager-kernels" || "${MODE}" == "wmma-profiler" || "${MODE}" == "batch1-decode" || "${MODE}" == "batch1-profile" || "${MODE}" == "batch1-graphs" ]]; then
