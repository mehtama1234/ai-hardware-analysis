...
receipt makes the contract boundary executable: all fifteen real targets now
have source-level behavioral contracts. In addition to the earlier targets,
committed contracts cover the asynchronous FIFO, Chameleon AHB scratchpad,
UART, both physical AIMC wrappers, the CSR peripheral, and the I2C GPIO
expander. The independent checker records `15/15` available contracts and the
inventory is ready for held-out execution. Compilation, timing, and physical
artifacts remain separate from behavioral qualification.
...
qualification.
The `.artifacts/heldout-openroad-issue-bundle-replay-20260915/` receipt
replays OpenROAD-flow-scripts commit `7b6f3669…`: the parent-generated runner
cannot survive extraction because it sources variables from the original
absolute `WORK_HOME`, while the corrected runner changes to its own directory
