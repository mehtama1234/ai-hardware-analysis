...
<header style="padding:64px 0 8px">
  ...
  <p class="dek">Gated DeltaNet fades the whole board by a single number — too blunt, because different kinds of memory deserve different fates. Kimi Delta Attention (KDA) gives <b>every channel its own decay rate</b>. The board can hold some dimensions steady while clearing others in the very same step. That fine-grained control is Kimi Linear's headline move — the first linear attention to <em>beat</em> full attention under fair comparison.</p>
  <div class="run"><div class="rt">▶ We ran it · fine beats blunt</div>
  <p>On a task that needs "keep these channels <em>and</em> forget those," per-channel KDA scores <b>{KDA_GOAL:.2f}</b> — the best any single scalar can manage is <b>{BEST_SCALAR:.2f}</b>, because one number must either keep everything or forget everything. And interleaving 3 KDA layers per MLA layer cuts the KV cache by a flat <b>−{HY['rows'][0]['reduction_pct']:.0f}%</b> at every context length — the paper's number, from first principles.</p></div>
</header>
