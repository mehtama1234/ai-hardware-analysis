...
module heldout_tb;
    ...
    reg [1:0] tile0_health_action = 0, tile1_health_action = 0;
    reg [1:0] tile2_health_action = 0, tile3_health_action = 0;
    ...
    aimc_scheduler_governor_pipelined dut (.*);
    always #5 clk = ~clk;
    ...
    initial begin
        ...
        if (final_decision !== 2'd1 || selected_tile !== 2'd2 || final_reason !== 4'd9) begin
            $display("FAIL requested tile contract decision=%0d tile=%0d reason=%0d", final_decision, selected_tile, final_reason);
        end else begin
            $display("PASS heldout pipelined requested tile contract");
        end
        $finish;
    end
endmodule
