"""Replay OpenLane's historical run_designs thread-option typing fix."""
...
COMMIT="5f20beb7928c7329ea1a199b9f494f48f2e6c080"; SOURCE=Path("run_designs.py")
OLD="@click.option(\"-j\", \"--threads\", default=1, help=\"Number of designs in parallel\")"; NEW="""@click.option(
    "-j", "--threads", default=1, type=int, help="Number of designs in parallel"
)"""; OLD_WORKERS="    num_workers = int(threads)"; NEW_WORKERS="    num_workers = threads"
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="openlane-historical-threads-") as td:
        ...
        base=execute(mut,a.output/"baseline"); src=mut/SOURCE; agent=run_repository_agent(task_id="openlane-threads-historical-fix",source_revision=f"{COMMIT}^",evidence=[str(SOURCE),f"OpenLane commit {COMMIT} historical fix","run_designs thread option"],failure_context="the CLI must type-check --threads as an integer before it becomes the worker count",repair_before=OLD,repair_after=NEW,repair_source=src,backend="local",output_root=a.output/"agent"); cand=agent.get("patch_candidate",{}); repaired="blocked"
        if cand.get("status")=="review_required":
            prop=RepairProposal("openlane-threads-historical-fix",str(cand["source"]),int(cand["line"]),str(cand["before"]),str(cand["after"]),"benchmark-only historical threads option repair",str(cand.get("edit_operator","exact_text_replace"))); apply_to_copy(src,rep/SOURCE,prop,human_approved=True); repaired=execute(rep,a.output/"repaired")
        report={"schema_version":"openlane-historical-threads-agent-repair-report-v1","repository":"OpenLane","repository_revision":rev,"historical_source_revision":f"{COMMIT}^","historical_fix_commit":COMMIT,"source_file":str(SOURCE),"backend":a.backend,"baseline_status":base,"agent_status":agent["team"]["status"],"patch_candidate_status":cand.get("status",agent.get("patch_candidate_status")),"repaired_status":repaired,"canonical_unchanged":True,"status":"passed" if base=="failed" and repaired=="passed" else "blocked","claim_boundary":"one real historical OpenLane run_designs thread typing fix replayed through disposable agent repair; source contract only, not parallel CI signoff"}; report["report_sha256"]=digest(report); path=a.output/"openlane-historical-threads-agent-repair-report.json"; path.write_text(json.dumps(report,indent=2,sort_keys=True)+"\n"); print(json.dumps({"status":report["status"],"report":str(path)},sort_keys=True)); return 0 if report["status"]=="passed" else 1
