#!/usr/bin/env python3
"""Run a real held-out replay on the physical tile-service scheduler."""
...
DESIGN = Path("designs/aimc_tile_service_scheduler_physical/src")
SOURCE = DESIGN / "aimc_tile_service_scheduler_physical.v"
TB = ROOT / "evidence/heldout-behavioral-contracts/aimc_tile_service_scheduler_physical/heldout_tb.v"
OLD = "            requested_tile_id_q <= requested_tile_id;"
NEW = "            requested_tile_id_q <= 8'd0;"
...
def main() -> int:
    ...
    if fixed.count(OLD) != 1: raise SystemExit("physical tile scheduler mutation anchor is not unique")
    ...
    with tempfile.TemporaryDirectory(prefix="heldout-tile-scheduler-physical-causal-") as temporary:
        ...
        canonical_values, mutated_values = signal_values(canonical_waveform, "selected_tile"), signal_values(mutated_waveform, "selected_tile")
        times = sorted({time for time, _ in canonical_values + mutated_values})
        frontier = state_frontier(held(mutated_values, times), held(canonical_values, times), signal="selected_tile")
        mutated_source = mutated / SOURCE; lines = mutated_source.read_text().splitlines(); mutation_line = next(i for i, line in enumerate(lines, 1) if "requested_tile_id_q <= 8'd0" in line)
        events = trace_events(mutated_waveform, ["selected_tile", "service_decision", "reason"])
        events.append(TraceEvent(f"e{len(events)}", "selected_tile", int(frontier["time"]), str(frontier["observed"])))
        graph = build_causal_graph(events, {"selected_tile": {"requested_tile_id_q"}, "requested_tile_id_q": set()}, driver_locations={("selected_tile", "requested_tile_id_q"): [{"file": str(mutated_source), "line": mutation_line}]})
        graph.update({"waveform": str(mutated_waveform), "rtl": str(mutated_source), "signals": ["selected_tile", "service_decision", "reason"], "rtl_sha256": hashlib.sha256(mutated_source.read_bytes()).hexdigest()}); graph["graph_sha256"] = digest({k: v for k, v in graph.items() if k != "graph_sha256"})
        ...
        report = {"schema_version": "heldout-real-tile-scheduler-physical-causal-localization-v1", "target": "OpenLane aimc_tile_service_scheduler_physical", "repository_revision": subprocess.run(["git", "-C", str(REPO), "rev-parse", "HEAD"], capture_output=True, text=True, check=True).stdout.strip(), "contract": str(TB), "contract_sha256": hashlib.sha256(TB.read_bytes()).hexdigest(), "canonical_status": canonical_status, "mutated_status": mutated_status, "frontier": frontier, "graph": graph, "binding": binding, "localization": localization, "timeline": timeline, "mutation_location": {"file": str(mutated_source), "line": mutation_line, "text": lines[mutation_line - 1].strip()}, "integrity_errors": errors, "status": "passed" if canonical_status == "passed" and mutated_status == "failed" and frontier.get("status") == "diverged" and binding.get("status") == "available" and localization.get("status") == "available" and timeline.get("status") == "available" and not errors else "blocked", "claim_boundary": "one held-out real OpenLane physical tile-service scheduler with source-level behavioral contract and seeded requested-tile capture mutation; module/cycle first-divergence evidence, not 90% generalization or complete protocol proof"}
        report["report_sha256"] = digest(report); path = args.output / "heldout-real-tile-scheduler-physical-causal-localization.json"; path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
