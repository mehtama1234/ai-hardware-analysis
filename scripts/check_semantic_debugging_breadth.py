...
def main():
 ...
 if r.get("status") not in {"passed", "blocked_pending_heldout_localization"}: errors.append("unsupported held-out status")
 ...
 m=r.get("metrics",{});
 if m.get("observed_localization_rate")!=1.0 or m.get("heldout_localization_rate")!=1.0 or m.get("heldout_localization_passed")!=m.get("heldout_localization_total") or m.get("required_heldout_designs")!=10: errors.append("localization boundary is malformed")
 ...
 if r.get("status")=="passed" and (m.get("heldout_localization_total",0)<m.get("required_heldout_designs",10) or "authenticated model repair generalization" not in r.get("claim_boundary","")): errors.append("passed boundary overclaims semantic evidence")
 if r.get("status")=="blocked_pending_heldout_localization" and (m.get("heldout_localization_total",0)>=m.get("required_heldout_designs",10) or "blocked" not in r.get("claim_boundary","").lower()): errors.append("blocked boundary is malformed")
