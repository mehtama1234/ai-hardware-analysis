...
def main():
 ...
 m=r.get("metrics",{});
 if m.get("observed_localization_rate")!=1.0 or m.get("heldout_localization_rate") is not None or m.get("required_heldout_designs")!=10: errors.append("localization boundary is malformed")
 ...
 if "not measured" not in r.get("claim_boundary","").lower() or "90%" not in r.get("claim_boundary",""): errors.append("claim boundary does not preserve held-out limit")
