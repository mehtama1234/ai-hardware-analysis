...
def main():
 ...
 m=r.get("metrics",{});
 if m.get("observed_localization_rate")!=1.0 or m.get("heldout_localization_passed")!=5 or m.get("heldout_localization_total")!=5 or m.get("heldout_localization_rate")!=1.0 or m.get("required_heldout_designs")!=10: errors.append("localization boundary is malformed")
 ...
 if "5/10" not in r.get("claim_boundary","") or "90%" not in r.get("claim_boundary","") or "blocked" not in r.get("claim_boundary","").lower(): errors.append("claim boundary does not preserve held-out limit")
