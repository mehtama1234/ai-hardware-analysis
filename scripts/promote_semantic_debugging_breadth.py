...
def main() -> int:
    ...
    required = metrics["required_heldout_designs"]
    localization_gate_passed = measured_total >= required and measured_passed / measured_total >= 0.9
    status = "passed" if localization_gate_passed else "blocked_pending_heldout_localization"
    boundary = (f"{catalog.get('target_count')} real multi-module designs are cataloged and four observed causal trajectories are checked; "
                f"{measured_passed}/{measured_total} measured held-out real targets pass causal localization. The semantic localization gate passes, "
                "but this receipt does not claim authenticated model repair generalization or physical/silicon signoff.") if localization_gate_passed else (f"{catalog.get('target_count')} real multi-module designs are cataloged and four observed causal trajectories are checked; {measured_passed}/{measured_total} measured held-out real targets pass causal localization, but only {measured_total}/{required} required held-out designs are measured, so the 90% held-out gate and generalization claim remain blocked.")
    receipt={"schema_version":"semantic-debugging-breadth-v1","generated_at":datetime.now(timezone.utc).isoformat(),"status":status,"artifacts":artifacts,"metrics":metrics,"claim_boundary":boundary}; receipt["receipt_sha256"]=hashlib.sha256(json.dumps(receipt,sort_keys=True,separators=(",",":")).encode()).hexdigest(); (out/"receipt.json").write_text(json.dumps(receipt,indent=2,sort_keys=True)+"\n"); print(json.dumps({"status":receipt["status"],"metrics":metrics,"output":str(out)},sort_keys=True)); return 0
