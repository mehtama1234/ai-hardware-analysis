"""Replay OpenROAD's historical CTS snapshot-label fix."""
from __future__ import annotations
import argparse,hashlib,json,os,subprocess,sys,tempfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];SUBREPO=ROOT/"analog-digital-chip-design-eda";REPO=Path("/home/mehtama1/eda-tools/OpenROAD-flow-scripts");COMMIT="53d01015";SOURCE=Path("flow/scripts/cts.tcl");OLD="save_progress 4_1_pre_repair_hold_setup";NEW="save_progress 4_1_pre_repair_setup_hold"
def digest(v):return hashlib.sha256(json.dumps(v,sort_keys=True,separators=(",",":")).encode()).hexdigest()
def execute(root,out):
 out.mkdir(parents=True,exist_ok=True);text=(root/SOURCE).read_text();ok=NEW in text and OLD not in text;(out/"snapshot-contract.txt").write_text("setup_hold\n" if ok else "hold_setup\n");return "passed" if ok else "failed"
def main():
 ...
 with tempfile.TemporaryDirectory(prefix="openroad-historical-cts-snapshot-") as td:
  ...
  baseline=execute(mut,a.output/"baseline");src=mut/SOURCE;agent=run_repository_agent(task_id="openroad-cts-snapshot-historical-fix",source_revision=f"{COMMIT}^",evidence=[str(SOURCE),f"OpenROAD-flow-scripts commit {COMMIT}","CTS progress snapshot"],failure_context="CTS pre-repair snapshot must use the canonical setup-hold checkpoint name",repair_before=OLD,repair_after=NEW,repair_source=src,backend="local",output_root=a.output/"agent");cand=agent.get("patch_candidate",{});repaired="blocked"
  if cand.get("status")=="review_required":
   prop=RepairProposal("openroad-cts-snapshot-historical-fix",str(cand["source"]),int(cand["line"]),str(cand["before"]),str(cand["after"]),"benchmark-only historical CTS snapshot repair",str(cand.get("edit_operator","exact_text_replace")));apply_to_copy(src,rep/SOURCE,prop,human_approved=True);repaired=execute(rep,a.output/"repaired")
  report={"schema_version":"openroad-historical-cts-snapshot-agent-repair-report-v1","repository":"OpenROAD-flow-scripts","repository_revision":rev,"historical_source_revision":f"{COMMIT}^","historical_fix_commit":COMMIT,"source_file":str(SOURCE),"backend":a.backend,"baseline_status":baseline,"agent_status":agent["team"]["status"],"patch_candidate_status":cand.get("status",agent.get("patch_candidate_status")),"repaired_status":repaired,"canonical_unchanged":True,"status":"passed" if baseline=="failed" and repaired=="passed" else "blocked","claim_boundary":"one real historical OpenROAD CTS checkpoint-label fix replayed through disposable agent repair; source contract, not full timing signoff"};report["report_sha256"]=digest(report);path=a.output/"openroad-historical-cts-snapshot-agent-repair-report.json";path.write_text(json.dumps(report,indent=2,sort_keys=True)+"\n");print(json.dumps({"status":report["status"],"report":str(path)},sort_keys=True));return 0 if report["status"]=="passed" else 1
