...
GENERATOR = LABS_ROOT / "analog" / "analog-in-memory-foundation-model-hardware" / "python" / "multi_tile_scheduler_runtime.py"
CSV_PATH = LABS_ROOT / "analog" / "analog-in-memory-foundation-model-hardware" / "measurements" / "multi-tile-scheduler-runtime.csv"
...
EXPECTED_FROM_CSV = {
    "tile_actions": "tile_actions_before",
    "busy": "tile_busy",
...
def expected_name(row: dict[str, str]) -> str:
    return f"token_{int(row['token']):02d}_{row['reason']}"
...
def main() -> int:
    ...
    compiled = run(
        [
            "iverilog",
            "-o",
            "aimc_tile_service_scheduler_tb",
            "aimc_tile_service_scheduler.v",
            "aimc_tile_service_scheduler_tb.v",
        ],
        HERE,
    )
    ...
    simulated = run(["vvp", "aimc_tile_service_scheduler_tb"], HERE)
    ...
    if failures:
        print("FAIL generated_scheduler_trace")
        for failure in failures:
            print(failure)
        return 1

    print("PASS generated_scheduler_trace")
