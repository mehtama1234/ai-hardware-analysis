...
GENERATOR = LABS_ROOT / "analog" / "analog-in-memory-foundation-model-hardware" / "python" / "error_budget_governor_trace.py"
CSV_PATH = LABS_ROOT / "analog" / "analog-in-memory-foundation-model-hardware" / "measurements" / "error-budget-governor-trace.csv"
...
def main() -> int:
    ...
    compiled = run(
        [
            "iverilog",
            "-o",
            "aimc_error_budget_governor_tb",
            "aimc_error_budget_governor.v",
            "aimc_error_budget_governor_tb.v",
        ],
        HERE,
    )
    ...
    simulated = run(["vvp", "aimc_error_budget_governor_tb"], HERE)
    ...
    if failures:
        print("FAIL generated_error_budget_governor_trace")
        for failure in failures:
            print(failure)
        return 1

    print("PASS generated_error_budget_governor_trace")
