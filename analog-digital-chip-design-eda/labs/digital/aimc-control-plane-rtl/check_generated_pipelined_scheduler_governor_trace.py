...
def main() -> int:
    ...
    compiled = run(
        [
            "iverilog",
            "-o",
            "aimc_scheduler_governor_pipelined_tb",
            "aimc_tile_service_scheduler.v",
            "aimc_error_budget_governor.v",
            "aimc_scheduler_governor.v",
            "aimc_scheduler_governor_pipelined.v",
            "aimc_scheduler_governor_pipelined_tb.v",
        ],
        HERE,
    )
    ...
    simulated = run(["vvp", "aimc_scheduler_governor_pipelined_tb"], HERE)
    ...
    if failures:
        print("FAIL generated_pipelined_scheduler_governor_trace")
        for failure in failures:
            print(failure)
        return 1

    print("PASS generated_pipelined_scheduler_governor_trace")
