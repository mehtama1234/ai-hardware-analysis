...
def main():
    ...
    parser.add_argument('--release-ns',type=float,default=8)
    parser.add_argument('--input-diffs-mv',type=float,nargs='+',default=[-100.,100.,-0.1529705854,0.1529705854])
    args=parser.parse_args();args.output.mkdir(parents=True,exist_ok=False)
    if not 1 <= args.release_ns <= 100: raise ValueError('release must be within 1–100 ns')
    netlist=args.netlist.resolve()
    times=[args.release_ns+d for d in [-0.1,0.3,10,20,40,90]]
    nodes=['decision_p','decision_n','latch_sense_p_ext','latch_sense_n_ext','tail_ext','iso_tail_ext','preamp_iso_tail_ext']
    cases=[]
    for i,diff in enumerate(args.input_diffs_mv):
        case=args.output/f'case-{i}';case.mkdir()
        text=deck(netlist,diff,'extracted').replace('30n 60n)','200n 400n)').replace('20n 30n)','200n 400n)').replace('.tran 20p 20n uic',f'.tran 20p {args.release_ns+92}n uic')
        text=text.replace('8.00n 20p',f'{args.release_ns}n 20p').replace('8.20n 20p',f'{args.release_ns+0.2}n 20p')
        probes=[]
    ...
    report={'schema_version':'converter_regeneration_hold_diagnostic.v1','netlist':str(netlist),
            ...
            'protocol':{'hold_until_ns':args.release_ns+92,'reset_release_ns':args.release_ns,'evaluate_enable_ns':args.release_ns+0.2,'sample_times_ns':times,
                        'sense_load_ohms':float(os.environ.get('AIMC_SENSE_LOAD_OHM','100000')),
