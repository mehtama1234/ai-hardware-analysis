...
def schematic(source,isolate_drains=False):
    lines=source.splitlines()
    changed=[]
    for i,line in enumerate(lines):
        if line.startswith('+ iso_tail_ext tail_ext vdd_ext preamp_iso_tail_ext'):
            lines[i]=line+' sample_clock regen_bar'+(' sample_bar' if isolate_drains else '')
        fields=line.split()
        ...
        if fields[0] in ('X4','X6'):
            ...
            if isolate_drains:
                fields[1:4]=[{'decision_p':'sample_drain_p','decision_n':'sample_drain_n'}.get(n,n) for n in fields[1:4]]
...
'''
    if isolate_drains:
        for side in ['p','n']:
            extra+=f'Xacquire_n{side} decision_{side} sample_clock sample_drain_{side} vss_escape sky130_fd_pr__nfet_01v8 w=6 l=0.6\n'
            extra+=f'Xacquire_p{side} decision_{side} sample_bar sample_drain_{side} vdd_ext sky130_fd_pr__pfet_01v8 w=6 l=0.6\n'
    text='\n'.join(lines)+'\n'
...
def main():
    ...
    parser.add_argument('--isolate-drains',action='store_true')
    parser.add_argument('--sample-durations-ns',type=float,nargs='+',default=[0.1,0.3,1.0])
    args=parser.parse_args();out=args.output.resolve();out.mkdir(parents=True,exist_ok=False)
    if any(not 0<d<=2 for d in args.sample_durations_ns):raise ValueError('sample durations must be positive and at most 2 ns')
    original=args.netlist.resolve();new=out/'separated_sample_regeneration.spice';new.write_text(schematic(original.read_text(),args.isolate_drains))
    ...
    cases=[]
    for duration in args.sample_durations_ns:
        for diff in [-100.,100.,-0.1529705854,0.1529705854]:
            ...
            text=text.replace('preamp_iso_tail_ext aimc_converter_macro_active_candidate_flat',
                              'preamp_iso_tail_ext sample_clock regen_bar'+(' sample_bar' if args.isolate_drains else '')+' aimc_converter_macro_active_candidate_flat')
...
VREGEN regen_bar 0 PULSE({{vdd}} 0 {regen}n 20p 20p 20n 60n)''')
            if args.isolate_drains:
                text=text.replace('RTAIL tail_ext 0 100G',f'RTAIL tail_ext 0 100G\nVSAMPLEBAR sample_bar 0 PULSE({{vdd}} 0 8.2n 20p 20p {duration}n 60n)')
...
'''
    ...
    report={'schema_version':'separated_sample_regeneration_experiment.v1','evidence_kind':'schematic_topology_experiment',
            ...
            'drain_isolation_transmission_gates':args.isolate_drains,
