...
def main():
    ...
    for line in (run/"circuit.spice").read_text().splitlines():
        if not line.startswith("X"):
            continue
        if line.startswith("XCHARGE"):
            fields=line.split()
            if not contract.get("modeled_charge_caps") or fields[3:]!=["sky130_fd_pr__cap_mim_m3_1","l=2","w=2"]:
                raise ValueError("Unexpected capacitor integration")
            name,top,bottom=fields[:3]
            voltages=[]
            for node in (top,bottom):
                if node not in nodes:
                    nodes[node]=nodes["xu."+node] if "xu."+node in nodes else ideal_source(deck,node,time)
                voltages.append(nodes[node])
            voltage=voltages[0]-voltages[1]
            cap_rows.append({"device":name,"top":top,"bottom":bottom,"min_terminal_difference_v":float(voltage.min()),
                             "max_terminal_difference_v":float(voltage.max()),"qualification":False})
            continue
    ...
    if contract.get("modeled_charge_caps"):
        expected=[("a_1561_36413#","vdd_capture"),("a_1975_36413#","vdd_capture"),
                  ("a_1592_36047#","vss"),("a_2017_36047#","vss")]
        if [(r["device"],r["top"],r["bottom"]) for r in cap_rows]!=[(f"XCHARGE{i}",a,b) for i,(a,b) in enumerate(expected)]:
            raise ValueError("Modeled capacitor terminal/count mismatch")
    ...
    result={"status":"terminal_range_audit_complete_not_qualification","source_run":str(run),
            "source_implementation_boundary":contract.get("boundary"),
            "source_combined_layout_lvs_verified":contract.get("combined_layout_lvs_verified",False),
            ...
            "sources":sources,"special_nfet_alias_evidence":alias_evidence,"devices":rows,"modeled_capacitor_terminal_observations":cap_rows,
