...
def geometry(active_width_um=1.2, input_width_um=1.2):
    ...
    for y, pfet, source_net in rows:
        for side in (-1, 1):
            ...
            device_width_um = active_width_um if y == 9000 else input_width_um if y in (3000, 6000) else 1.2
            diffusion_top = 690 + round(device_width_um*100)
            r("pdiffusion" if pfet else "ndiffusion", 3100, 690, 3700, diffusion_top)
            ...
            if y == 0:
                # Expose each sampled input on a dedicated M4 escape track.
                # The original pin remains at the gate for cell-level LVS;
                # the escape lets a parent macro route outside the output
                # trunks instead of crossing them.
                via(g, y+280, 1, 4)
                wire("metal4", g, y+280, side*6000, y+280)
                via(side*6000, y+280, 3, 4)
    ...
    for side in (-1, 1):
        ...
        # Give each regenerated output a short M4 edge escape.  Parent
        # macros must not route across the latch core's reset/supply tracks.
        via(side*2000, 4000, 1, 4)
        wire("metal4", side*2000, 4000, side*6000, 4000)
        via(side*6000, 4000, 3, 4)
        port("out_p" if side == -1 else "out_n", side*2000, 4000)
    # Dedicated parent-macro escapes for controls and rails.  Keep these
    # tracks outside the latch core so a hierarchical wrapper can drive the
    # extracted cell without landing on an internal feedback trunk.
    control_escapes = [
        ("latch_sense_p", -1500, 1000, -6000),
        ("latch_sense_n", 1500, 1000, 6000),
        ("iso_tail", 0, -500, -6000),
        ("tail", 0, 15800, 6000),
        ("reset", 0, 12280, -6000),
        ("eval", 0, 15080, -6000),
        ("vdd_active", 0, 10500, 6000),
        ("vdd", 0, 13500, 6000),
        ("vss", -1000, 17000, -6000),
    ]
    for _name, x, y, edge_x in control_escapes:
        via(x, y, 2, 4)
        wire("metal4", x, y, edge_x, y)
        via(edge_x, y, 3, 4)
        # Publish the edge landing as the formal cell port.  This preserves
        # the electrical net and ordering while making hierarchical parent
        # extraction bind to the accessible escape, not the core pin.
        port(_name, edge_x, y, "metal4")
...
def main():
    ...
    parser.add_argument("--active-width-um", type=float, choices=(1.2, 2.4), default=1.2)
    parser.add_argument("--input-width-um", type=float, choices=(1.2, 2.4, 4.8, 6.0), default=1.2)
    ...
    mag = out / f"{CELL}.mag"
    mag.write_text(geometry(args.active_width_um, args.input_width_um))
    reference = (ROOT/"labs/analog/analog-in-memory-foundation-model-hardware/spice/sky130_isolated_latch_v2_reference.spice").read_text()
    reference_lines = reference.splitlines()
    for index, line in enumerate(reference_lines):
        if line.startswith(("Xsense_p ", "Xsense_n ", "Xfeedback_p ", "Xfeedback_n ")):
            reference_lines[index] = line.replace("w=1.2", f"w={args.input_width_um}")
        elif line.startswith(("Xload_p ", "Xload_n ")):
            reference_lines[index] = line.replace("w=1.2", f"w={args.active_width_um}")
    ...
    report = {"status": "layout_candidate_incomplete", "drc_errors": int(counts[-1]) if counts else None,
              "active_load_width_um": args.active_width_um, "input_feedback_width_um": args.input_width_um,
    ...
    report["sha256"] = {name: hashlib.sha256((out/name).read_bytes()).hexdigest()
                        for name in (f"{CELL}.mag", f"{CELL}.ext", "extracted.spice", "reference.spice", "magic.log", "commands.tcl")
                        if (out/name).exists()}
