...
def geometry(active_width_um=1.2):
    ...
    for y, pfet, source_net in rows:
        for side in (-1, 1):
            ...
            diffusion_top = 690 + round(active_width_um*100) if y == 9000 else 810
...
def main():
    ...
    mag = out / f"{CELL}.mag"
    mag.write_text(geometry(args.active_width_um))
    ...
    for index, line in enumerate(reference_lines):
        if line.startswith(("Xload_p ", "Xload_n ")):
    ...
    report = {"status": "layout_candidate_incomplete", "drc_errors": int(counts[-1]) if counts else None,
              "active_load_width_um": args.active_width_um,
