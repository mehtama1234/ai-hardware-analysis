...
def main():
    ...
    reference='''* Intended active macro: existing independent latch circuit and added differential pair.
.include latch_reference.spice
.subckt aimc_converter_macro_active_candidate_flat vss vdd row_drive sar_comparator_input
...
{chr(10).join(f'Xpreamp_p_{index} latch_sense_p_ext row_drive preamp_iso_tail_ext vss_escape sky130_fd_pr__nfet_01v8 w=1.2 l=0.6' for index in range(args.preamp_fingers))}
{chr(10).join(f'Xpreamp_n_{index} latch_sense_n_ext sar_comparator_input preamp_iso_tail_ext vss_escape sky130_fd_pr__nfet_01v8 w=1.2 l=0.6' for index in range(args.preamp_fingers))}
.ends aimc_converter_macro_active_candidate_flat
'''
    ...
    cell='aimc_converter_macro_active_candidate_flat'
    ...
    report={'status':'active_transistor_macro_lvs_pass' if passed else 'active_transistor_macro_lvs_failed',
            ...
            'preamp_fingers':args.preamp_fingers,
