...
def main():
    ...
    parser.add_argument("--capture-fall-intervention",action="store_true",
                        help="Compare only a fall-slew intervention with identical solver settings")
    ...
    for run_path,(contract,result,deck) in zip(paths[1:],loaded[1:]):
        if args.capture_fall_intervention:
            from run_latch_capture_integration import capture_clock_source
            for path,c in ((paths[0],base_c),(run_path,contract)):
                exact=(path/"deck.spice").read_text()
                expected="VCAP capture_clk 0 "+capture_clock_source(c["capture_clock_rise_ps"],
                    c["negative_control_capture_clock_disabled"],c.get("capture_clock_fall_ps",100))
                if expected not in exact.splitlines():
                    raise ValueError("Clock contract/deck mismatch")
            for key in ("solver","capture_clock_rise_ps","capture_50pct_ns","negative_control_capture_clock_disabled"):
                if contract[key]!=base_c[key]:
                    raise ValueError("Non-fall parameter changed")
            deck=re.sub(r"(?m)^VCAP .*\n","",deck)
            comparison_base=re.sub(r"(?m)^VCAP .*\n","",base_deck)
        else:
            comparison_base=base_deck
        if deck!=comparison_base or contract["sha256"]["circuit.spice"]!=base_c["sha256"]["circuit.spice"]:
    ...
    if args.capture_fall_intervention:
        shutil.copy2(ROOT/"scripts/run_latch_capture_integration.py",out/"clock_source_reference.py")
    (out/"result.json").write_text(json.dumps({"status":"capture_fall_intervention_only" if args.capture_fall_intervention else "solver_comparison_only","accepted_converter":False,
