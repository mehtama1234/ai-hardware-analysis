...
def circuit(initial,final=None,capacitance=100e-15,load=1e12,resistor=10000,switch_scale=1):
    ...
    for bit in range(8):
        ...
        for label,gate,terminal,model,width,bulk in [
            ...
            lines.append(f"X{bit}{label} bit{bit} {gate} {terminal} {bulk} sky130_fd_pr__{model}_01v8 w={width*switch_scale:.12g} l=0.15")
        lines.append(f"Rbranch{bit} n{bit} bit{bit} {2*resistor:.12g}")
        if bit<7:lines.append(f"Rseries{bit} n{bit} n{bit+1} {resistor:.12g}")
    lines += [f"Rterminate n7 0 {2*resistor:.12g}",f"Rload n0 0 {load:.12g}",f"Cload n0 0 {capacitance:.12g}",
...
def transfer_deck(codes,load,resistor=10000,switch_scale=1):
    ...
    return circuit(0,load=load,resistor=resistor,switch_scale=switch_scale)+"\n".join(control)+"\n"
...
def run(args):
    if not np.isfinite(args.resistor_ohm) or args.resistor_ohm<=0 or not 0<args.switch_scale<=10:
        raise ValueError("Positive resistor and switch scale in (0,10] required")
    ...
    protocol={"kind":"nominal_schematic_characterization","switch_corner":"SKY130 TT","temperature_c":27,
              "vdd_v":1.8,"reference_source_v":1.2,"bits":8,"R_ohms":args.resistor_ohm,"two_R_ohms":2*args.resistor_ohm,
              "switch_nfet_w_l_um":[4*args.switch_scale,.15],"switch_pfet_w_l_um":[8*args.switch_scale,.15],"switch_transistors":32,
    ...
    for load in protocol['dc_loads_ohm']:
        ...
        log,meta=simulate(args.output/f'dc-{load:g}',transfer_deck(codes,load,args.resistor_ohm,args.switch_scale))
    ...
    if not args.smoke:
        ...
        for cap in protocol['transient_load_capacitances_f']:
            for before,after in protocol['transitions']:
                ...
                deck=circuit(before,after,capacitance=cap,resistor=args.resistor_ohm,switch_scale=args.switch_scale)+".tran 20p 220n\n.control\nset numdgt=15\nset wr_singlescale\nset wr_vecnames\nrun\nwrdata waveform.txt v(n0)\nquit\n.endc\n.end\n"
...
if __name__=='__main__':
    ...
    p.add_argument('--resistor-ohm',type=float,default=10000)
    p.add_argument('--switch-scale',type=float,default=1)
    run(p.parse_args())
