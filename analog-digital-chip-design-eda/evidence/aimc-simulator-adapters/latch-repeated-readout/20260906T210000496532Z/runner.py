...
def stimulus(differences, polarity, setup_ns=5):
    value = lambda diff: 0.9 + polarity * diff / 2000
    points = [(0, value(differences[0]))]
    for index, diff in enumerate(differences[1:], 1):
        points.extend([(50*index-setup_ns, value(differences[index-1])),
                       (50*index-setup_ns+0.02, value(diff))])
...
def main():
    ...
    deck = replace_line(deck, "VP sense_p ", "VP sense_p 0 " + stimulus(applied_differences, 1, args.input_setup_ns))
    deck = replace_line(deck, "VN sense_n ", "VN sense_n 0 " + stimulus(applied_differences, -1, args.input_setup_ns))
    deck = replace_line(deck, ".tran ", f".tran 20p {50*args.cycles}n")
    deck = replace_line(deck, "VRESET reset ", f"VRESET reset 0 PULSE(0 {args.reset_high_v:.12g} 8n 20p {args.clock_fall_ps:.12g}p {30-args.reset_early_ns:.12g}n 50n)")
