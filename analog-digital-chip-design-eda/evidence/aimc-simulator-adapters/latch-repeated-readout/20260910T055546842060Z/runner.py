...
def output_reset_source(high_v,rise_ps,fall_ps,advance_ns,period_ns):
    # Keep falling-edge start at the historical 38.02-advance ns offset.
    # A rise-time experiment must not silently move the precharge edge too.
    width_ns=30-advance_ns+(20-rise_ps)/1000
    return f"VRESET reset 0 PULSE(0 {high_v:.12g} 8n {rise_ps:.12g}p {fall_ps:.12g}p {width_ns:.12g}n {period_ns:.12g}n)"
...
def main():
    ...
    parser.add_argument("--reset-rise-ps",type=float,choices=(20,100,500,1000),default=20,
                        help="Output reset release slew only; preserve precharge edge start")
    parser.add_argument("--reset-fall-ps",type=float,choices=(500,1000,2000),
                        help="Output precharge slew only; defaults to clock-fall-ps")
    ...
    parser.add_argument("--max-step-ps", type=float, choices=(5,10,20),
                        help="Explicit transient maximum step for numerical sensitivity checks")
    parser.add_argument("--reltol", type=float, choices=(1e-4,1e-5), default=1e-4)
    parser.add_argument("--equalizer-release-ns",type=float,
                        help="Release the physical internal equalizer before evaluation; requires equalizer layout")
    args = parser.parse_args()
    if args.equalizer_release_ns is not None and (not np.isfinite(args.equalizer_release_ns) or not 0.5<=args.equalizer_release_ns<=7):
        parser.error("equalizer release must be within 0.5..7 ns")
    if args.equalizer_release_ns is not None and not args.combined_layout:
        parser.error("equalizer timing requires a combined equalizer layout")
    ...
    if not np.isfinite(args.timeout_s) or not 1 <= args.timeout_s <= 600:
    ...
    if args.combined_layout:
        ...
        if bool(physical.get("internal_equalizer")) != (args.equalizer_release_ns is not None):
            raise ValueError("Physical equalizer and explicit control timing must be selected together")
    ...
    if args.combined_layout:
        extra_port=" eq_reset" if args.equalizer_release_ns is not None else ""
        deck = replace_line(deck, "XU ", "XU out_p out_n latch_sense_p latch_sense_n tail reset vdd vss eval sense_p sense_n iso_tail vdd_active rx_p rx_n"+extra_port+" combined_latch_receiver")
        if args.equalizer_release_ns is not None:
            width=38-args.reset_early_ns-args.equalizer_release_ns
            deck=deck.replace("XU out_p ",f"VEQRESET eq_reset 0 PULSE(0 1.8 {args.equalizer_release_ns:.12g}n 20p {args.clock_fall_ps:.12g}p {width:.12g}n {args.period_ns:.12g}n)\nXU out_p ",1)
    ...
    if args.max_step_ps is not None:
        deck = replace_line(deck, ".tran ", f".tran 20p {args.period_ns*args.cycles:.12g}n 0 {args.max_step_ps:.12g}p")
    deck = replace_line(deck, ".options method=", f".options method=gear reltol={args.reltol:.12g} abstol=1e-14 vntol=1e-7")
    reset_fall_ps=args.clock_fall_ps if args.reset_fall_ps is None else args.reset_fall_ps
    deck = replace_line(deck, "VRESET reset ", output_reset_source(args.reset_high_v,args.reset_rise_ps,reset_fall_ps,args.reset_early_ns,args.period_ns))
    ...
    if args.equalizer_release_ns is not None:
        vectors.append("v(eq_reset)")
    vectors += ["v(sense_p)","v(sense_n)","i(vp)","i(vn)","i(vreset)","i(veval)"]
    if args.equalizer_release_ns is not None:
        vectors.append("i(veqreset)")
    if args.receiver:
    ...
    deck = replace_line(deck, "write waveform.raw ", "write waveform.raw " + " ".join(vectors[1:]))
    (out/"deck.spice").write_text(deck)
    contract = {"source_run": str(source), "input_diffs_mv": differences, "period_ns": args.period_ns,
                "max_step_ps": args.max_step_ps, "reltol": args.reltol,
                "equalizer_release_ns":args.equalizer_release_ns,
                "equalizer_control_boundary":"Ideal external clock, not an implemented driver" if args.equalizer_release_ns is not None else None,
                ...
                "reset_rise_ps":args.reset_rise_ps,
                "reset_fall_ps":reset_fall_ps,
                "reset_high_v": args.reset_high_v,
                "clock_input_current_observables_saved":True,
    ...
    try:
        ...
        legal = all(
            extrema[name][0] >= 0 and extrema[name][1] <= 1.8
        ...
        result = {"status": "repeated_diagnostic_pass" if all(row["pass"] for row in rows) else "repeated_diagnostic_fail",
                  "waveform_legal": legal, "waveform_legality_scope": legal_names,
