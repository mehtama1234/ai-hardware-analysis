...
def add_internal_charge_caps(netlist,cap_ff):
    ...
    if len(ends)!=1 or "CCHARGE" in netlist:
...
def main():
    ...
    if args.internal_charge_cap_ff and not args.combined_layout:
    ...
    if args.internal_charge_cap_ff:
        shutil.copy2(out/"circuit.spice",out/"source_extracted.spice")
        (out/"circuit.spice").write_text(add_internal_charge_caps((out/"circuit.spice").read_text(),args.internal_charge_cap_ff))
...
'''
    ...
    contract={"source_latch":str(LATCH),"source_capture":str(CAPTURE),"temperature_c":25,"corner":"tt","mismatch_enabled":False,
              ...
              "combined_layout_lvs_verified":bool(combined) and not bool(args.internal_charge_cap_ff),
