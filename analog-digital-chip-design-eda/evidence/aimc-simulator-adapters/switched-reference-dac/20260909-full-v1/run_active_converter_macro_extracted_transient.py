...
def deck(netlist: Path, diff_mv: float, view: str) -> str:
    half = diff_mv / 2000.0
    import os
    swapped = os.environ.get("AIMC_SWAP_INPUTS", "0") == "1"
    row = 0.9 + half if not swapped else 0.9 - half
    comparator = 0.9 - half if not swapped else 0.9 + half
    reset_start = float(os.environ.get("AIMC_RESET_START_NS", "8.00"))
    eval_start = float(os.environ.get("AIMC_EVAL_START_NS", "8.20"))
    ...
    iso_tail_bias_a = float(__import__("os").environ.get("AIMC_ISO_TAIL_BIAS_A", "4e-6"))
...
VRESET reset 0 PULSE(0 {{vdd}} {reset_start:.6g}n 20p 20p 30n 60n)
VEVAL eval 0 PULSE(0 {{vdd}} {eval_start:.6g}n 20p 20p 20n 30n)
VSSX vss_escape 0 0
VDD_EXT vdd_ext 0 {{vdd}}
RP vdd_ext latch_sense_p_ext {sense_load_p_ohm:.12g}
RN vdd_ext latch_sense_n_ext {sense_load_n_ohm:.12g}
IBIAS iso_tail_ext 0 {iso_tail_bias_a:.12g}
{preamp_bias_line}
...
def main() -> int:
    ...
    report = {
        ...
        "input_assignment_swapped": __import__("os").environ.get("AIMC_SWAP_INPUTS", "0") == "1",
        "reset_start_ns": float(__import__("os").environ.get("AIMC_RESET_START_NS", "8.00")),
        "eval_start_ns": float(__import__("os").environ.get("AIMC_EVAL_START_NS", "8.20")),
