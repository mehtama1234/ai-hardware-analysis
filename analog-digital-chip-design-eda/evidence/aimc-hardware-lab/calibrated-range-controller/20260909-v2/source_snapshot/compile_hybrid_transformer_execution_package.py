...
def validate_range_lowering(row: dict[str, object]) -> None:
    """Never silently drop a calibrated analog range requirement."""
    if row["placement"] == "analog_memory" and row.get("adc_range_contract") is not None:
        raise ValueError("unsupported_adc_range_programming: review ISA has no range configuration or readiness operation; use digital fallback")
