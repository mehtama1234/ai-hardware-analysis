...
def run(args):
    ...
    for index,row in enumerate(contract["adc_range"]["ranges"]):
        ...
        gain=normalized_gain(a,float(step),bits,bound)
        ...
        normalized_current=(x[:,r:r+nr]/a)@(qcode.astype(np.float64)/maximum)
    ...
    report={"schema_version":"calibrated_array_normalized_scaling.v1",
            ...
            "physical_equations":{"row_voltage":"V_drive_full_scale * quantized_input / activation_bound",
            ...
            "rounding_check":{"training_vector_indices":sample_indices,"tile_count":len(scales),"tile_vector_cases":len(scales)*len(sample_indices),
                              ...
                              "scope":"four quantized training vectors per tile; not whole-model physical evaluation",
                              "residual_cause":"ideal equispaced real-valued conductance codes versus rounded float32 weight products"},
