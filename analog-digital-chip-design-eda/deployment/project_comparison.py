...
def compare_project_runs(baseline_root: str | Path, retest_root: str | Path) -> dict[str, Any]:
    ...
    # A repair intentionally changes the RTL artifact. The test stimulus must
    # remain the same for a baseline/retest claim to be comparable.
    scope_comparable = baseline.get("testbench_artifact_id") == retest.get("testbench_artifact_id")
    result: dict[str, Any] = {
        ...
        "metrics": {
            ...
            "failure_resolved": (baseline["execution"]["status"] == "failed" and retest["execution"]["status"] == "passed") if scope_comparable else None,
            "coverage_delta_percentage_points": round(retest["coverage"]["percentage"] - baseline["coverage"]["percentage"], 4) if scope_comparable else None,
        ...
        "claim_boundary": "comparison measures captured simulation runs; it does not establish functional coverage, formal proof, or hardware qualification" if scope_comparable else "baseline and retest scopes differ; results are incomparable and cannot establish failure resolution or closure",
    }
