#!/usr/bin/env python3
"""Join seeded-timeout model repair, formal proof, and OpenLane evidence."""
...
def main() -> int:
    ...
    run, source = args.physical_run.resolve(), args.repaired_source.resolve()
    model, repair, formal = [json.loads(path.read_text(encoding="utf-8")) for path in (model_path, repair_path, formal_path)]
    metrics = next(csv.DictReader((run / "reports/metrics.csv").open(encoding="utf-8")))
    staged = run.parent.parent / "src/timeout.sv"
    ...
    model_passed = model.get("timeout_repair_run", {}).get("status") == "available" and model.get("timeout_repair_run", {}).get("repair_match") is True
    evidence = {
        "schema_version": "seeded-timeout-model-repair-rtl2gds-handoff-v1",
        ...
        "generated_at": datetime.now(timezone.utc).isoformat(), "design": "seeded_timeout",
        "model": {"path": str(model_path), "sha256": digest(model_path), "normal_cases": 11, "repair_match": model.get("timeout_repair_run", {}).get("repair_match"), "grounded": model.get("timeout_repair_run", {}).get("grounded"), "operator": model.get("timeout_repair_run", {}).get("edit_operator")},
