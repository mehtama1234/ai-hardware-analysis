...
def main() -> int:
    ...
    for marker in ["aihwkit==1.1.0", "--no-deps", "github.com/sandialabs/cross-sim", "aimc-simulators-venv", "check_aimc_simulator_adapters.py"]:
    ...
    candidate_non_scaffold_count = candidate_workspace.get("non_scaffold_file_count")
    if not isinstance(candidate_non_scaffold_count, int) or candidate_non_scaffold_count < 2:
        return fail("converter post-layout real artifact discovery should show at least two current partial candidate files")
    ...
    for marker in ["Converter Post-Layout Real Artifact Discovery", "no_complete_real_artifact_package_found", "accepted post-layout exists: <code>False</code>", f"candidate non-scaffold file count: <code>{candidate_non_scaffold_count}</code>", "current candidate would write accepted evidence: <code>False</code>", "current candidate strict issue count: <code>22</code>", "Repo-Local Artifact Counts", "A real post-layout claim needs a chain of named objects", "Refused Claim"]:
    ...
    current_discovery_count = current_discovery.get("candidate_non_scaffold_file_count")
    if not isinstance(current_discovery_count, int) or current_discovery_count < 2:
        return fail("current AIMC system state summary should record current partial candidate files in real artifact discovery")
