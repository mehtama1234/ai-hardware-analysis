...
def main():
    ...
    parser.add_argument("--all-saved-nodes",action="store_true",
                        help="Include capture/internal nodes without changing the voltage envelope")
    ...
    reports={}
    selected=[name for name in names if name.startswith("v(")] if args.all_saved_nodes else ["v(rx_p)","v(rx_n)"]
    for receiver in selected:
        driver="v(out_p)" if receiver=="v(rx_p)" else "v(out_n)"
        ...
        for event in events:
            ...
            event.update(peak_ns=ns,cycle=int(ns//contract["period_ns"]),offset_ns=ns%contract["period_ns"],
                coincident_v={name:float(wave[index,names.index(name)]) for name in
                    (driver,"v(reset)","v(eval)","v(eq_reset)","v(capture_clk)","v(rx_n)") if name in names})
    ...
    report={"source_run":str(run),"receivers":reports,"all_saved_nodes":args.all_saved_nodes,"accepted_converter":False,
