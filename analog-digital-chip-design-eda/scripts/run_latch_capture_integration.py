...
def add_internal_charge_caps(netlist,cap_ff,modeled=False):
    ...
    label="* Exploratory modeled MIM caps: NOT part of source LVS extraction" if modeled else "* Exploratory lumped caps: NOT part of source LVS extraction"
    lines[ends[0]:ends[0]]=[label]+additions
    return "\n".join(lines)+"\n"
...
def main():
    ...
    if cap_probe:
        shutil.copy2(cap_probe/"result.json",out/"capacitor_model_preflight.json")
...
'''
    ...
    if charge_modified:
        ...
        if args.modeled_charge_caps:
            ...
            contract["sha256"]["capacitor_model_preflight.json"]=hashlib.sha256((out/"capacitor_model_preflight.json").read_bytes()).hexdigest()
