...
def make_program(descriptors,banks=8,lanes=16,columns=128,epoch_base=0):
    if banks<1 or lanes<1 or columns<1 or epoch_base<0:
    ...
    for start in range(0,len(descriptors),banks):
        ...
        epoch=epoch_base+start//banks+1
        for operation in ["select_tile","configure_range","range_ready","load_dac","array_ready"]:
            for bank,descriptor in enumerate(active):
                events.append({"operation":operation,"bank":bank,"tile":descriptor["tile_id"],"binding_epoch":epoch})
        for round_index in range((columns+lanes-1)//lanes):
            for bank,descriptor in enumerate(active):
                events.append({"operation":"read_adc","bank":bank,"tile":descriptor["tile_id"],
                               "binding_epoch":epoch,"round":round_index,"columns":min(lanes,columns-round_index*lanes)})
        for bank,descriptor in enumerate(active):
            events.append({"operation":"accumulate","bank":bank,"tile":descriptor["tile_id"],"binding_epoch":epoch})
    return events
...
def replay(program,descriptors,banks=8,lanes=16,columns=128):
    ...
    for event in program:
        ...
        epoch=event["binding_epoch"]
        if not isinstance(epoch,int) or epoch<1:
            raise ValueError("Invalid binding epoch")
        if not 0<=bank<banks or tile not in expected_tiles:
            raise ValueError("Unknown bank or tile")
        if operation=="select_tile":
            if bank in states and epoch<=states[bank]["epoch"]:
                raise ValueError("Binding epoch reused")
            ...
            states[bank]={"tile":tile,"epoch":epoch,"phase":"selected","round":0,"columns":0}
            continue
        state=states.get(bank)
        if state is None or state["tile"]!=tile or state["epoch"]!=epoch:
