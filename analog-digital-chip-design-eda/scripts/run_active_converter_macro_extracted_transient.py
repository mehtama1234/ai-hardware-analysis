...
def deck(netlist: Path, diff_mv: float, view: str) -> str:
    ...
    decision_time_ns = float(__import__("os").environ.get("AIMC_DECISION_TIME_NS", "18.0"))
    stop_time_ns = float(__import__("os").environ.get("AIMC_STOP_TIME_NS", "20.0"))
    if not 8.2 < decision_time_ns < stop_time_ns <= 120.0:
        raise ValueError("AIMC_DECISION_TIME_NS must be after evaluation start and before AIMC_STOP_TIME_NS")
...
.tran 20p {stop_time_ns:.12g}n uic
.measure tran decision_p_v FIND v(decision_p) AT={decision_time_ns:.12g}n
.measure tran decision_n_v FIND v(decision_n) AT={decision_time_ns:.12g}n
.measure tran decision_diff_v PARAM='decision_n_v-decision_p_v'
.measure tran supply_charge_c INTEG I(VDD) FROM=0 TO={stop_time_ns:.12g}n
.measure tran active_supply_charge_c INTEG I(VDD_EXT) FROM=0 TO={stop_time_ns:.12g}n
.control
run
.endc
.end
...
def main() -> int:
    ...
    if not 0 < args.timeout_seconds <= 120:
        raise SystemExit("timeout must be positive and at most 120 seconds")
    ...
    hypothetical = "HYPOTHETICAL CONNECTIVITY DIAGNOSTIC" in netlist.read_text().splitlines()[0]
    ...
    for index, diff in enumerate(args.input_diffs_mv):
        ...
        try:
            proc = subprocess.run(["ngspice", "-b", str(case_deck)], cwd=case, text=True, capture_output=True, check=False, timeout=args.timeout_seconds)
            timed_out = False
        ...
        if proc.returncode == 0:
            ...
            row.update({"decision_p_v": measure(proc.stdout, "decision_p_v"), "decision_n_v": measure(proc.stdout, "decision_n_v"), "decision_diff_v": output_diff,
                        ...
                        "energy_scope": "signed net energy delivered by VDD and VDD_EXT over the configured transient window; excludes input/clock driver and bias-generation costs",
    ...
    report = {
        ...
        "evidence_kind": "hypothetical_netlist_reconnection" if hypothetical else "extracted_netlist_transient",
        "claim_boundary": {
    ...
    if hypothetical:
        report["status"] = "hypothetical_connectivity_diagnostic_not_layout_evidence"
        report["claim_boundary"]["allowed"] = "bounded simulation of a hypothetical node reconnection; no corresponding layout revision"
