...
'''
    ...
    if proc.returncode==0:
        ...
        if names!=["time","v(top)","i(vin)"] or not np.isfinite(wave).all() or wave[-1,0]<4e-9 or not np.all(np.diff(wave[:,0])>0):
        ...
        expected_ff=2*(2-.025)**2+.19*4*(2-.025)
        result.update(nominal_formula_capacitance_ff=expected_ff,
                      nominal_formula_scope="Inspected nominal area/perimeter coefficients 2 fF/um2, 0.19 fF/um and -0.025 um dimension correction; not a corner/yield claim",
                      nominal_formula_agreement=abs(charge/1e-15-expected_ff)<1e-4)
    else:
